...
############# output file ##############
    ...
    for i in range(num_emps):
        data1 = "\n" + emp[i] + "  " + job[i] + format(gross_pay[i], currency) + tab + format(fed_tax[i], currency)  + tab + format(state_tax[i], currency) + tab + format(soc_sec[i], currency) + format(medicare[i], currency) + tab + format(net_pay[i], currency)
        f.write(data1)   
    f.write(line)
